  ...
  NestedFunctionOutput:
    ...
    Value: !Select
      ...
      - !Split
        ...
        it('YAML GetAtt autocomplete should include documentation', () => {
            const template = new TemplateBuilder(DocumentType.YAML);
            const scenario: TemplateScenario = {
                name: 'GetAtt documentation test',
                steps: [
                    {
                        action: 'type',
                        content: `AWSTemplateFormatVersion: '2010-09-09'
Description: 'Test template for GetAtt autocomplete bug in YAML'
...
Resources:
  MyS3Bucket:
    Type: AWS::S3::Bucket
    Properties:
      BucketName: !Sub '\${BucketPrefix}-\${AWS::StackName}'
...
Outputs:  
  S3BucketArn:
    Description: 'S3 Bucket ARN - should show: Arn, DomainName, RegionalDomainName, WebsiteURL'`,
                        position: { line: 0, character: 0 },
                    },
                    {
                        action: 'type',
                        content: `
    Value: !GetAtt MyS3Bucket.`,
                        position: { line: 73, character: 95 },
                        description: 'GetAtt autocomplete should include documentation for S3 attributes',
                        verification: {
                            position: { line: 74, character: 30 },
                            expectation: CompletionExpectationBuilder.create()
                                .expectContainsItems(['Arn', 'DomainName', 'RegionalDomainName', 'WebsiteURL'])
                                .build(),
                        },
                    },
                ],
