...
async function updateHookConfiguration(
    ...
    return await components.hooksManager.runExclusiveForType(typeName, () =>
        applyHookConfigurationUpdate(components, typeName, mutate),
    );
}

async function applyHookConfigurationUpdate(
    components: HooksComponents,
    typeName: string,
    mutate: (hookConfiguration: Record<string, unknown>) => void,
): Promise<string | undefined> {
...
export function validateRuleHandler(
    components: HooksComponents,
): RequestHandler<ValidateRuleParams, ValidateRuleResult, void> {
    return (params) => handleValidateRule(components, params);
}

function handleValidateRule(components: HooksComponents, params: ValidateRuleParams): ValidateRuleResult {
    try {
        const parsed = parseWithPrettyError(parseValidateRuleParams, params);
        const result = components.guardEngine.validateRule(parsed.ruleContent, parsed.sampleTemplate);
        return {
            valid: result.valid,
...
export function uploadRuleHandler(
    ...
    return async (params) => {
        try {
            ...
            components.hooksManager.invalidateRuleContent(parsed.s3Uri);
    ...
    return async (params) => {
        try {
            ...
            return await previewGuardHooks(
                {
                    listHooksDetailed: () => components.hooksManager.listAllHooksDetailed(),
                    ...
                    accountId,
                    evaluateRule: (evaluateParams) => components.guardEngine.evaluateRule(evaluateParams),
                },
