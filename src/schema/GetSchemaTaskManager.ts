...
export class GetSchemaTaskManager implements SettingsConfigurable, Closeable {
    ...
    constructor(
        ...
        private readonly schemaRetriever?: { rebuildAffectedCombinedSchemas(region?: string, profile?: string): void },
    ) {
    ...
    runPrivateTask() {
        this.privateTask
            ...
            .then(() => {
                this.schemaRetriever?.rebuildAffectedCombinedSchemas(undefined, this.profile);
            })
            .catch(() => {});
    }
    ...
    runSamTask() {
        this.samTask
            ...
            .then(() => {
                this.schemaRetriever?.rebuildAffectedCombinedSchemas(); // No params = SAM update
            })
            .catch(() => {});
    }
    ...
    private run() {
        ...
        if (task) {
            task.run(this.schemas.publicSchemas, this.log)
                .then(() => {
                    this.schemaRetriever?.rebuildAffectedCombinedSchemas(task.region);
                })
                .catch(() => {
