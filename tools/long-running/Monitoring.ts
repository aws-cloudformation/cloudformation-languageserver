...
const metrics: Record<OperationType, StandaloneTestMetrics> = {} as Record<OperationType, StandaloneTestMetrics>;
...
export function logProgress(): void {
    ...
    console.log(`   Success Rate: ${totalOps > 0 ? (((totalOps - totalFailed) / totalOps) * 100).toFixed(1) : 0}%`);
    ...
    for (const [operationType, metric] of Object.entries(metrics) as [OperationType, StandaloneTestMetrics][]) {
        if (metric.operationsAttempted > 0) {
            const successRate = (
                ((metric.operationsAttempted - metric.operationsFailed) / metric.operationsAttempted) *
                100
            ).toFixed(1);
            console.log(
                `   ${operationType}: ${metric.operationsAttempted} ops, ${metric.operationsFailed} failed (${successRate}% success)`,
            );
        }
    }
}
...
export function generateFinalReport(testStartTime: number): void {
    ...
    console.log(`Successful: ${totalOps - totalFailed}`);
    console.log(`Failed: ${totalFailed}`);
    console.log(`Success Rate: ${totalOps > 0 ? (((totalOps - totalFailed) / totalOps) * 100).toFixed(2) : 0}%`);
