...
export class LspClient implements LspConnection {
    ...
    async getSystemStatus(): Promise<any> {
        ...
        //console.log('LspClient: Requesting system status...');
        const result = await this.connection.sendRequest('aws/cfn/system/status', {});
        //console.log('LspClient: System status received:', JSON.stringify(result, null, 2));
        return result;
    }

    async checkSchemaReadiness(region: string): Promise<{ region: string; schemasReady: boolean }> {
        if (!this.connection) {
            throw new Error('LSP client not initialized');
        }
